...
def validate_sample(sample: Dict) -> Optional[Dict]:
    sys_msg = (
        ...
        "1. grounded: true HANYA JIKA completion murni dari dokumen. False jika ada halusinasi atau tambahan opini luar.\n"
        ...
        "Output JSON:\n"
        '{"pass": boolean, "grounded": boolean, "oracle_consistency": boolean, "thought_correct": boolean, "answer_type_correct": boolean, "score": float}'
    )
...
def run_raft_pipeline(output_path: Path):
    ...
    for doc_id, chunk in tqdm(valid_oracles, desc="Generating RAFT Dataset"):
        ...
        for attempt in range(3): # Coba maksimal 3 kali jika tidak pass
            ...
            if validation_res and validation_res.get("pass", False) and validation_res.get("score", 0) >= 8.0:
