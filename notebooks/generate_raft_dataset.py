...
OUTPUT_FILE = DATASET_DIR / "raft_perdes_dataset.jsonl"
REPAIRED_FILE = DATASET_DIR / "raft_perdes_dataset_repaired.jsonl"
...
class GeneratorConfig:
    ...
    chunks_per_doc: Optional[int] = 1  # None = full
...
def load_all_chunks(processed_dir: Path) -> List[Dict[str, Any]]:
    """Load all *_chunks.json files from processed_dir and return combined list."""
    all_chunks: List[Dict[str, Any]] = []
    chunk_files = sorted(processed_dir.glob("*_chunks.json"))
    if not chunk_files:
        raise FileNotFoundError(f"No *_chunks.json files found in {processed_dir}")
    for fpath in chunk_files:
        with open(fpath, "r", encoding="utf-8") as f:
            chunks = json.load(f)
        print(f"  Loaded {len(chunks):>4} chunks from: {fpath.name}")
        all_chunks.extend(chunks)
    print(f"  Total: {len(all_chunks)} chunks from {len(chunk_files)} files")
    return all_chunks
